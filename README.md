...
Every other name matches watcher's `display_name` exactly — including five that read like typos and are not: `Layer 6`, whose space is watcher's and the lab's own; `Grafana Labs`, which is the `grafana` board's full `display_name`; `Tripadvisor`, whose lowercase "a" is watcher's and the company's own branding; `Flipp`, whose doubled "p" is the company's own spelling; and `Fullscript`, which is one word with a lowercase "s". Do not close up "Layer6", shorten "Grafana", camel-case "TripAdvisor", drop a "p" from "Flip", or split "Full Script": each would make another divergence, silently.
